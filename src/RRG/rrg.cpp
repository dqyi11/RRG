...
RRGNode::RRGNode(POS2D pos) {
    m_pos = pos;
    m_cost = 0.0;
    mp_parent = NULL;
}

bool RRGNode::operator==(const RRGNode &other) {
...
RRG::RRG( int width, int height, int segment_length ) {
...
RRGNode* RRG::init( POS2D start, POS2D goal, COST_FUNC_PTR p_func, double** pp_cost_distribution ) {
    if( _p_root ) {
        delete _p_root;
        _p_root = NULL;
    }
    ...
    _p_root = new RRGNode( start );
    _nodes.push_back(_p_root);
    root.setRRGNode(_p_root);
...
bool RRG::_is_in_obstacle( POS2D pos ) {
    int x = (int)pos[0];
    int y = (int)pos[1];
...
bool RRG::_is_obstacle_free( POS2D pos_a, POS2D pos_b ) {
    if ( pos_a == pos_b ) {
        return true;
    }
...
KDNode2D RRG::_find_nearest( POS2D pos ) {
    KDNode2D node( pos );
...
std::list<KDNode2D> RRG::_find_near(POS2D pos) {
...
double RRG::_calculate_cost( POS2D& pos_a, POS2D& pos_b ) {
    return _p_cost_func(pos_a, pos_b, _pp_cost_distribution, this);
}

RRGNode* RRG::_create_new_node(POS2D pos) {
    RRGNode * pNode = new RRGNode(pos);
    _nodes.push_back(pNode);

    return pNode;
}

bool RRG::_remove_edge(RRGNode* p_node_parent, RRGNode*  p_node_child) {
    ...
    bool removed = false;
    for( std::list<RRGNode*>::iterator it=p_node_parent->m_child_nodes.begin();it!=p_node_parent->m_child_nodes.end();it++ ) {
        RRGNode* p_current = (RRGNode*)(*it);
...
bool RRG::_has_edge(RRGNode* p_node_parent, RRGNode* p_node_child) {
    if ( p_node_parent == NULL || p_node_child == NULL ) {
        return false;
    }
    for( std::list<RRGNode*>::iterator it=p_node_parent->m_child_nodes.begin();it!=p_node_parent->m_child_nodes.end();it++ ) {
        RRGNode* p_curr_node = (*it);
...
bool RRG::_add_edge( RRGNode* p_node_parent, RRGNode* p_node_child ) {
...
std::list<RRGNode*> RRG::_find_all_children( RRGNode* p_node ) {
    int level = 0;
    bool finished = false;
    std::list<RRGNode*> child_list;

    std::list<RRGNode*> current_level_nodes;
    current_level_nodes.push_back( p_node );
    while( false==finished ) {
        std::list<RRGNode*> current_level_children;
        int child_list_num = child_list.size();

        for( std::list<RRGNode*>::iterator it=current_level_nodes.begin(); it!=current_level_nodes.end(); it++ ) {
            RRGNode* pCurrentNode = (*it);
            for( std::list<RRGNode*>::iterator itc=pCurrentNode->m_child_nodes.begin(); itc!=pCurrentNode->m_child_nodes.end();itc++ ) {
                RRGNode *p_child_node= (*itc);
                if(p_child_node) {
        ...
        if (current_level_children.size()==0) {
            ...
            for( std::list<RRGNode*>::iterator itt=current_level_children.begin();itt!=current_level_children.end();itt++ ) {
                RRGNode * pTempNode = (*itt);
                if( pTempNode ) {
...
void RRG::_update_cost_to_children( RRGNode* p_node, double delta_cost ) {
    std::list<RRGNode*> child_list = _find_all_children( p_node );
    for( std::list<RRGNode*>::iterator it = child_list.begin(); it != child_list.end();it++ ) {
        RRGNode* p_child_node = (*it);
        if( p_child_node ) {
...
bool RRG::_get_closet_to_goal( RRGNode*& p_node_closet_to_goal, double& delta_cost ) {
    bool found = false;
    ...
    for(std::list<KDNode2D>::iterator it=near_nodes.begin();
        ...
        KDNode2D kd_node = (*it);
        RRGNode* p_node = kd_node.getRRGNode();
...
void RRG::dump_distribution(std::string filename) {
    std::ofstream myfile;
