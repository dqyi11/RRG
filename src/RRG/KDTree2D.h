...
public:
    KDNode2D(value_type x, value_type y) : POS2D(x, y) { mpRRGNode = NULL; }
    KDNode2D(POS2D & pos) : POS2D(pos) { mpRRGNode = NULL; }
    void setRRGNode(RRGNode* pNode) { mpRRGNode = pNode; }
    RRGNode* getRRGNode() { return mpRRGNode; }
protected:
    RRGNode* mpRRGNode;
};
