...
void MainWindow::planPath() {
    ...
    mpRRG = new RRG(mpMap->width(), mpMap->height(), mpViz->m_PPInfo.m_segment_length);

    POS2D start(mpViz->m_PPInfo.m_start.x(), mpViz->m_PPInfo.m_start.y());
    POS2D goal(mpViz->m_PPInfo.m_goal.x(), mpViz->m_PPInfo.m_goal.y());

    mpRRG->init(start, goal, mpViz->m_PPInfo.mp_func, mpViz->m_PPInfo.mCostDistribution);
    mpViz->m_PPInfo.get_obstacle_info(mpRRG->get_map_info());
    mpViz->setTree(mpRRG);

    mpRRG->dump_distribution("dist.txt");

    while(mpRRG->get_current_iteration() <= mpViz->m_PPInfo.m_max_iteration_num) {
        //QString msg = "CurrentIteration " + QString::number(mpRRG->get_current_iteration()) + " ";
        //msg += QString::number(mpRRG->get_ball_radius());
        //qDebug(msg.toStdString().c_str());
